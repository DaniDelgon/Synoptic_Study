...
name='gfdl_d01_1980_2009_slp_grEra5.nc'
#print(new_name(name))
# Definimos las variables
path='/Users/fredm/Desktop/TFG_DEFINITIVO/Bases_de_datos/Modelos_Climaticos/Pasado/Presion_superficial/'

def pres(path1,path2,name):
	ncin=Dataset('%s%s'%(path1,name),'r')
	#print(ncin)
	slp=ncin.variables['SLP'][:]    #CAMBIO PARA ALIGERAR TIEMPO DE COMPUTACION
	...
	Tipo 0: Híbridos (F<|Z|<2F) e indeterminados (F<3 & |Z|<3)
	...
	np.savetxt('%stipos_tiempo_mod_%s.txt'%(path2,new_name(name)),tt,delimiter=' , ',fmt='%.1f')

path1='/Users/fredm/Desktop/TFG_DEFINITIVO/Bases_de_datos/Modelos_Climaticos/Pasado/Presion_superficial/'
path2='/Users/fredm/Desktop/TFG_DEFINITIVO/Ficheros/Modelos_climaticos/Pasado/Presion_superficial/'
presion = [f for f in listdir(path1) if isfile(join(path1, f))]

[pres(path1,path2,i) for i in presion]
